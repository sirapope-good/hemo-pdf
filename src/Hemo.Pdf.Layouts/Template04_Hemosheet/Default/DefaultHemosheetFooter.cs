...
/// <summary>
/// Bottom band of clinical-03 Default hemosheet: complication/nursing + health/med/notes/vitals.
/// Kept separate from <see cref="DefaultHemosheetForm"/> so footer layout edits stay localized.
/// </summary>
internal static class DefaultHemosheetFooter
{
    ...
    public const float PageNumberFooterMm = 9f;

    /// <summary>Headroom so wrap / border / font metrics do not push footer to page 2.</summary>
    public const float LayoutSafetyMm = 14f;

    /// <summary>Post Vital + AVF + Dialysis Nurse + Dialysis NA (right column only).</summary>
    public static float RightStripBandHeightMm => 4f * HemosheetDefaultStyle.PostStripRowHeightMm;
    ...
    public static float NephrologistRowHeightMm => HemosheetDefaultStyle.PostStripRowHeightMm;
    ...
    public static void ComposeBand(IContainer c, HemosheetReportViewModel vm)
    {
        ...
        // Keep checks + notes + Neph + Post Vital…NA together when they fit; if the
        // remaining page is too short, let QuestPDF paginate rather than leaving an
        // empty orphan header page above a ShowEntire jump.
        c.Row(row =>
        {
    ...
    private static void NephrologistCell(IContainer c, HemosheetReportViewModel vm, float heightMm)
    {
        c.Border(Bw).Height(heightMm, Mm).AlignMiddle().AlignCenter()
            .Text(text =>
            {
                text.Span("Nephrologist ").Style(DefaultText.Bold);
                var doctor = vm.DoctorName ?? vm.Patient.DoctorName;
                text.Span(string.IsNullOrWhiteSpace(doctor) ? "-" : doctor).Style(DefaultText.Base);
            });
    }
    ...
    private static void RightLowerStrips(IContainer c, HemosheetReportViewModel vm)
    {
        c.Column(right =>
        {
            ...
            right.Item().Border(Bw).Height(HemosheetDefaultStyle.PostStripRowHeightMm, Mm).Row(r =>
            {
    ...
    /// <summary>How many wrapped lines a nurse-note display string needs (1..<see cref="HemosheetDefaultStyle.NurseNoteMaxLines"/>).</summary>
    internal static int EstimateNoteLines(string display)
    {
        ...
        return Math.Clamp(lines, 1, HemosheetDefaultStyle.NurseNoteMaxLines);
    }
    ...
    public static float HealthEducationHeightMm() =>
        ...
        + HealthItems.Length * HemosheetDefaultStyle.PostCheckRowHeightMm;
    ...
    public static float MedicationNaturalHeightMm(HemosheetReportViewModel vm)
    {
        ...
        if (lines <= 0) lines = 2;
        return HemosheetDefaultStyle.HeaderBarHeightMm + MedColHeaderMm + lines * Rh + Rh;
    }
    ...
    {
        var rh = HemosheetDefaultStyle.PostCheckRowHeightMm;
        var h = HemosheetDefaultStyle.HeaderBarHeightMm + items.Length * rh;
        if (items2 is not null)
            h += HemosheetDefaultStyle.HeaderBarHeightMm + items2.Length * rh;
        return h;
    }
        ...
        c.Column(col =>
        {
            ...
            foreach (var item in items)
            {
                col.Item().Height(rh, Mm).PaddingLeft(1f)
                    .CheckLine(item, ThaiUrDataHelper.Checked(vm, item));
            }
    ...
    {
        var minRh = HemosheetDefaultStyle.PostCheckRowHeightMm;
        c.Column(col =>
        {
    ...
    private static void MedicationTable(IContainer c, HemosheetReportViewModel vm, float targetHeightMm)
    {
        var lines = Math.Max(vm.LayoutContext.ReportSettings.FixedLines.Medicine, vm.MedicineRecords.Count);
        if (lines <= 0) lines = 2;
        var aboveHctMm = HemosheetDefaultStyle.HeaderBarHeightMm + MedColHeaderMm + lines * Rh;
        ...
        c.Column(col =>
        {
            ...
            col.Item().Table(t =>
            {
                ...
                t.Cell().Border(Bw).Height(MedColHeaderMm, Mm).AlignMiddle().AlignCenter()
                    .Text("Name/Dose/Route").Style(DefaultText.Bold);
                t.Cell().Border(Bw).Height(MedColHeaderMm, Mm).AlignMiddle().AlignCenter()
                    .Text("Time").Style(DefaultText.Bold);
                t.Cell().Border(Bw).Height(MedColHeaderMm, Mm).AlignMiddle().AlignCenter()
                    .Text("Sign").Style(DefaultText.Bold);

                for (var i = 0; i < lines; i++)
                {
                    var m = i < vm.MedicineRecords.Count ? vm.MedicineRecords[i] : null;
                    var name = m is null ? "" : $"{m.MedicineName} {ThaiUrDataHelper.Num(m.Quantity)} {m.Route}".Trim();
                    t.Cell().BorderLeft(Bw).BorderRight(Bw).MinHeight(Rh, Mm).PaddingLeft(1f).AlignMiddle()
                        .Text(name).Style(DefaultText.Base);
                    t.Cell().BorderRight(Bw).MinHeight(Rh, Mm).ValueCentered(ThaiUrDataHelper.Time(m?.Timestamp));
    ...
    {
        var p = vm.PostVital;
        var h = HemosheetDefaultStyle.PostStripRowHeightMm;
        c.Table(t =>
        {
            t.ColumnsDefinition(cols =>
            {
            ...
            t.Cell().Border(Bw).Height(h, Mm).AlignMiddle().PaddingLeft(1f)
                .Text("Post Vital").Style(DefaultText.Bold);
            PostVitalCell(t, h, "BP", ThaiUrDataHelper.Bp(p?.Bps, p?.Bpd));
            PostVitalCell(t, h, "PR", ThaiUrDataHelper.Num(p?.Hr));
            PostVitalCell(t, h, "RR", ThaiUrDataHelper.Num(p?.Rr));
            PostVitalCell(t, h, "BT", $"{ThaiUrDataHelper.Num(p?.Temp)} \u00B0C");
            PostVitalCell(t, h, "Sat", $"{ThaiUrDataHelper.Num(p?.SpO2)} %");
        });
    }
    ...
    private static void PostVitalCell(TableDescriptor t, float heightMm, string label, string value)
    {
        ...
        {
            inner.AutoItem().AlignMiddle().Text($"{label} ").Style(DefaultText.Bold);
            inner.RelativeItem().AlignMiddle()
                .Text(string.IsNullOrWhiteSpace(value) ? "-" : value).Style(DefaultText.Base);
        });
    }
    ...
    private static void AvfRow(IContainer c, HemosheetReportViewModel vm)
    {
        ...
        c.Table(t =>
        {
            ...
            t.Cell().Border(Bw).Height(h, Mm).AlignMiddle().PaddingLeft(1f)
                .Text("AVF/AVG").Style(DefaultText.Bold);
            AvfCell(t, h, "Thrill", ThaiUrDataHelper.PostState(vm, "thrill", "vas:av:thrill", "post:thrill"));
            AvfCell(t, h, "Bruit", ThaiUrDataHelper.PostState(vm, "bruit", "vas:av:bruit", "post:bruit"));
            AvfCell(t, h, "Hematoma", ThaiUrDataHelper.PostState(vm, "hematoma", "hema", "vas:hematoma"));
            AvfCell(t, h, "Stop Bleeding > 20 min",
                ThaiUrDataHelper.PostState(vm, "sb", "stop bleeding", "stopbleeding", "vas:sb"));
        });
    }
    ...
    private static void AvfCell(TableDescriptor t, float heightMm, string label, bool? yes)
    {
        ...
        {
            r.RelativeItem().AlignMiddle().Text(label).Style(DefaultText.Base);
            r.ConstantItem(2f);
