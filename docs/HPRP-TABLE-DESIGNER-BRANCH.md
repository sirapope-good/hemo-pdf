...
- Production **clinical-06-medication**: `layoutMode: designer` — ThaiUR header + **`data-grid`** Med History matrix (Medication / Frequency / Physician + **5** date columns with ✓/X; pad to ~22 med slots then Nurse / Pharmacist|Nephrologist; `chrome.rowHeightMm: 5.2`) + **`box-text`** review note (`bind: $.reviewNote`, align right). Second-signer label from backend `GlobalSetting.MedHistoryReport.SecondSignerRole`. Report-data from Web.Api `clinical-06-medication/report-data` (same Med History source as Telerik `medhistory`). Not a prescription list.
- Production **clinical-10-patient-data**: `layoutMode: designer` — ThaiUR header + **`group` section frame** (`chrome.border: thin`) wrapping **`field-row`** demographics/dialysis (checkbox options + fill-in; empty bind = blank form) + **`data-grid`** AV access. Outer frame prints even when children use `border: none`. Studio: select blocks → **ห่อ selection เป็นกรอบนอก**. Report-data from Web.Api `clinical-10-patient-data/report-data`.
- Production **clinical-11-admission**: `layoutMode: designer` — ThaiUR header + **`field-row`** admission/annual note (ข้อมูลพื้นฐาน, โรคร่วม, PE, สาเหตุ ESRD) flowing onto **≥2 pages** + **`data-grid`** AV access + HBV vaccine duo (`place: beside`) + other vaccines / meds / signatures + **`page-of`**. Empty bind = blank form. Report-data from Web.Api `clinical-11-admission/report-data`.
- Production **clinical-08/09 consent**: `layoutMode: designer` — ThaiUR header + dense `clinical.consent-narrative` (`contentMode: intro` / `closing`) + designer **`narrative`** (Word-lite body paragraphs; Studio Insert → Narrative). Pack paragraphs editable; optional `bindParagraphs: $.bodyParagraphs` overrides body at PDF time. Report-data adapter `consent` + nested `header` for preset binds.
