...
- Production **clinical-10-patient-data**: `layoutMode: designer` — ThaiUR header + **`field-row`** demographics/dialysis (checkbox options + fill-in; empty bind = blank form) + **`data-grid`** AV access history. Report-data from Web.Api `clinical-10-patient-data/report-data`. Fields without Hemopro storage (marital/religion/vaccines) are out of Phase A.
