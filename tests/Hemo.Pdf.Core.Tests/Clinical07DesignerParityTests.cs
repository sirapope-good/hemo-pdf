...
public class Clinical07DesignerParityTests
{
    static Clinical07DesignerParityTests()
    {
        QuestPDF.Settings.License = LicenseType.Community;
    }

    [Fact]
    public async Task Clinical07DesignerPackage_ValidatesAndRendersPdf()
    {
        var templatesRoot = HprpTestAssets.TemplatesRoot();
        var package = LoadClinical07Package(templatesRoot);
        ...
        var sample = HprpStudioSamplePayloads.TryLoad(templatesRoot, ClinicalReportCatalog.Lab);
    ...
    [Fact]
    public async Task PackClinical07_WritesPackageFile()
    {
        var templatesRoot = HprpTestAssets.TemplatesRoot();
        ...
        var packed = await pack.PackTemplateIdAsync(ClinicalReportCatalog.Lab);
        Assert.Single(packed);
        Assert.True(File.Exists(packed[0].OutputPath));
        Assert.EndsWith("clinical-07-lab.hprp", packed[0].OutputPath, StringComparison.OrdinalIgnoreCase);
    }

    private static HprpPackage LoadClinical07Package(string templatesRoot)
    {
        var dir = Path.Combine(templatesRoot, "reports", ClinicalReportCatalog.Lab);
